( function ( M, $ ) {
	...
	QUnit.test( '#render empty list', 4, function( assert ) {
		var $el = $( '<div>' );
		new Nearby( { latitude: 37.7, longitude: -122, range: 1000, el: $el } );
		assert.ok( this.spy.calledWithMatch(  { latitude: 37.7, longitude: -122 }, 1000  ), 'Check API got called' );
	...
	QUnit.test( '#render with location error', 4, function( assert ) {
		var $el = $( '<div>' ),
			n = new Nearby( { range: 1000, el: $el, errorType: 'location' } );
	...
	QUnit.test( '#render with current location', 2, function( assert ) {
		var $el = $( '<div>' );
		new Nearby( { useCurrentLocation: true, range: 1000, el: $el } );
	...
	QUnit.test( '#render with a server error', 3, function( assert ) {
		var $el = $( '<div>' ),
			n = new Nearby( { latitude: 37.7, longitude: -122, range: 1000, el: $el } );
		assert.ok( this.spy.calledWithMatch(  { latitude: 37.7, longitude: -122 }, 1000  ), 'Check API got called' );
		assert.strictEqual( $el.find( '.error' ).length, 1, 'Check error got rendered' );
		assert.strictEqual( $el.find( '.error h2' ).text(),
