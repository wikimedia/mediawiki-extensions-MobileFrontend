...
Then(/^I see the language overlay$/) do
  on(ArticlePage).language_overlay_element.should be_visible
end

When(/^I click the language overlay close button$/) do
  on(ArticlePage).language_overlay_close_button_element.click
end

Then(/^I don't see the languages overlay$/) do
  on(ArticlePage).language_overlay_element.should_not be_visible
end
